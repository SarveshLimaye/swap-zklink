...
import {SwapRouterLibrary} from "./libraries/SwapRouterLibrary.sol";
import {IWNativeToken} from "./interfaces/IWNativeToken.sol";
import {IERC20} from "./interfaces/IERC20.sol";
import {SafeMath} from "./libraries/SafeMath.sol";

contract LinkSwapClone is ISwapRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = SwapRouterLibrary.getReserves(
            _factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = SwapRouterLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint256 amountAOptimal = SwapRouterLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
        ...
        )
    {
        ...
        address pair = SwapRouterLibrary.pairFor(
            _factory,
            token,
            _WNativeToken
        );
        ...
        if (msg.value > amountNativeToken) {
            TransferHelper.safeTransferNativeToken(
                msg.sender,
                msg.value - amountNativeToken
            );
        }
    }
    ...
    function removeLiquidityNativeTokenSupportingFeeOnTransferTokens(
        ...
        returns (uint256 amountNativeToken)
    {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? SwapRouterLibrary.pairFor(_factory, output, path[i + 2])
                : _to;
            ISwapPair(SwapRouterLibrary.pairFor(_factory, input, output)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
        ...
        returns (uint256[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            SwapRouterLibrary.pairFor(_factory, path[0], path[1]),
            amounts[0]
        );
        ...
        returns (uint256[] memory amounts)
    {
        ...
        assert(
            IWNativeToken(_WNativeToken).transfer(
                SwapRouterLibrary.pairFor(_factory, path[0], path[1]),
                amounts[0]
            )
        );
    ...
    {
        require(
            path[path.length - 1] == _WNativeToken,
            "SwapRouter: INVALID_PATH"
        );
        amounts = SwapRouterLibrary.getAmountsIn(_factory, amountOut, path);
        require(
            amounts[0] <= amountInMax,
    ...
    function swapNativeTokenForExactTokens(
        uint256 amountOut,
        address[] calldata path,
        address to,
        uint256 deadline
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            ISwapPair pair = ISwapPair(
                SwapRouterLibrary.pairFor(_factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = SwapRouterLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
