...
contract SwapPair is ISwapPairBase, SwapERC20 {
    ...
    /// @dev Prevents calling a function from anyone except the address returned by ISwapFactory#owner()
    ...
    function swap(
        ...
        );
        {
            ...
            uint256 balance0Adjusted = balance0.mul(1000).sub(
                amount0In.mul(3 + swapFeeRate)
            );
            uint256 balance1Adjusted = balance1.mul(1000).sub(
                amount1In.mul(3 + swapFeeRate)
            );
            require(
    ...
    function setSwapFeeReceipt(
        address _swapFeeReceipt
